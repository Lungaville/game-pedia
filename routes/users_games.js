...
function checkGameExists(id_game){
 ...
 function checkUserGameUnique(id_user,id_game){
  return model.users_games.findOne({ where : { id_user : id_user,id_game:id_game }});
}
router.post('/game',[
    ...
    if(await checkUserGameUnique(req.user_auth.id,req.body.id_game) !=null){
    ...
    try {
      ...
      if (user_game) {
        return    response.insert(res,'Successfuly inserted user game',user_game);
      }
     } catch (err) {
